...
The first step is to build a blast database for your reference genome using MagicBLAST. MagicBLAST produces a SAM file, which is processed by separate scripts encoding samtools commands. The SAM file is converted to a BAM file, which is sorted and indexed. A quality control report is generated by running FastQC on the BAM file.  The Subread tool featureCounts is then used to produce gene counts, which are then normalized with TMM and converted to gene expression in R with the packages limma and edgeR. The resulting normalized gene expression, dot plots, violin plots, heatmap, and FastQC report are displayed on the GUI.  

# How to use viSRAtoo
The misRA.R script works by taking in 3 arguments (2 SRR numbers for mouse RNA-seq data and 1 list of genes), processes the files and outputs gene dot plots into the working directory.  The shiny scripts (server.R ui.R) are still in development.
